...
class KGCDataset(Dataset):
    def __init__(self, config, split="train"):
        ...
        if self.use_desc:
            ...
            self.description_separator = "<extra_id_4>"
    ...
    def get_ent_rel_alias_dicts(self, dataset_name: str) -> Tuple[Dict, Dict]:
        ent_fname = os.path.join(os.getcwd(), 'data', dataset_name, 'entity_mentions.del')
        rel_fname = os.path.join(os.getcwd(), 'data', dataset_name, 'relation_mentions.del')
...
class KGCContextDataset(KGCDataset):
    def __init__(self, config, split="train"):
        ...
        self.context_separator = "<extra_id_2>"
        if self.is_legacy:
            self.context_separator = "\n"
        self.drop_mask_token = "<extra_id_1>"
        self.context_hop_separator = "<extra_id_3>"
    ...
    def create_query_string(self, triple, split=None):
        ...
        if random.random() >= self.drop_subject_percentage:
            source = 'query: ' + self.ent_aliases[triple[0]] + '\n'
        else:
            source = 'query: ' + self.drop_mask_token + '\n'
        target = 'query: ' + self.ent_aliases[triple[0]] + '\n'
        if self.use_desc:
    ...
    def triple_context_to_source_target(self, triple, context_list, split=None):
        ...
        if self.use_context:
            source, target = self.create_query_string(triple, split=split)
        else:
        ...
        context_size = 0
        k_id = 10
        while '<extra_id_10>' not in source:
            for p, o in context_list[:self.max_context_size]:
                p = self.rel_aliases[p]
                o = self.ent_aliases[o]
                if random.random() < 0.15:
                    mask_token = f'<extra_id_{k_id}>'
                    if random.random() >= 0.5:
                        s_target = mask_token + p + " "
                        p = mask_token
                    else:
                        s_target = mask_token + o + " "
                        o = mask_token
                    target += s_target
                    k_id += 1
                if k_id==99:
                ...
                # target += f"{self.context_separator} {p2}{sep}{o2}"
                context_size += 1
    ...
    def get(self, idx: int, split: str = "train") -> Dict:
        ...
        context_list = self.get_context(triple[0])
...
class KGCV1Dataset(KGCDataset):
    def __init__(self, config, split):
        super().__init__(config=config, split=split)
        self.tail_pred_token = "<extra_id_5>"
        self.head_pred_token = "<extra_id_6>"
